<template>
  <v-card>
    ...
    <v-card-text class="py-0">
      <div v-for="subfeature in subfeatures" :key="`subfeature-${subfeature.subfeature}`"
        class="d-flex align-center bg-tile-default rounded mb-2 pa-2" :class="{ 'pl-0': subfeatures.length > 1 }">
        <v-icon v-if="subfeatures.length > 1" class="mr-2" :color="subfeature.isCurrent ? 'primary' : undefined"
          x-large>mdi-roman-numeral-{{ subfeature.subfeature + 1 }}</v-icon>
        <div class="flex-grow-1">
          <gb-tooltip>
            <template v-slot:activator="{ on, attrs }">
              <v-progress-linear class="rounded" height="20" :value="subfeature.expPercent" v-bind="attrs" v-on="on">{{
                $vuetify.lang.t('$vuetify.gooboo.level') }} {{ $formatInt(subfeature.level) }} ({{
                  $formatNum(subfeature.expPercent, true) }}%)</v-progress-linear>
            </template>
            <template v-if="subfeature.expGain > 0">
              <div>{{ $vuetify.lang.t(`$vuetify.cryolab.expDescription`, $formatNum(subfeature.exp),
                $formatNum(subfeature.expNeeded), $formatNum(subfeature.expGain)) }}</div>
              <div>{{ $vuetify.lang.t(`$vuetify.cryolab.expDescription2`, $formatInt(subfeature.expGL)) }}</div>
              <alert-text type="formula" v-if="showFormulas">{{ $vuetify.lang.t(`$vuetify.cryolab.expFormula`)
                }}</alert-text>
              <div v-if="subfeature.nextLevelTime">{{ $vuetify.lang.t(`$vuetify.cryolab.nextLevelTime`,
                $formatTime(subfeature.nextLevelTime)) }}</div>
            </template>
            ...
            <div>
              <display-row v-for="(item, key) in subfeature.effect" :key="`${item.name}-${item.type}-${key}`"
                :name="item.name" :type="item.type" :before="item.before" :after="item.after"></display-row>
            </div>
          </gb-tooltip>
          <div class="d-flex justify-center align-center mt-1">
            <gb-tooltip :title-text="$vuetify.lang.t(`$vuetify.cryolab.passiveTitle`)">
              <template v-slot:activator="{ on, attrs }">
                <div :class="{ 'primary--text': !(feature.active && subfeature.isCurrent) }" v-bind="attrs" v-on="on">{{
                  $vuetify.lang.t('$vuetify.cryolab.passive', $formatNum(subfeature.passiveMult * 100, true)) }}</div>
              </template>
              <div>{{ $vuetify.lang.t('$vuetify.cryolab.passiveDescription', $formatNum(subfeature.passiveMult * 100,
                true)) }}
              </div>
            ...
            <gb-tooltip :title-text="$vuetify.lang.t(`$vuetify.cryolab.activeTitle`)">
              <template v-slot:activator="{ on, attrs }">
                <div :class="{ 'primary--text': feature.active && subfeature.isCurrent }" v-bind="attrs" v-on="on">{{
                  $vuetify.lang.t('$vuetify.cryolab.active', $formatNum(subfeature.activeMult * 100, true)) }}</div>
              </template>
              <div>{{ $vuetify.lang.t('$vuetify.cryolab.activeDescription', $formatNum(subfeature.activeMult * 100,
                true)) }}
              </div>
      ...
      <div class="d-flex flex-wrap mx-n1">
        <template v-for="(amount, currency) in prestigeGain">
          <gb-tooltip v-if="currency === 'farm_exp'" :key="`currency-gain-${currency}`" :min-width="0">
            <template v-slot:activator="{ on, attrs }">
              <v-chip class="ma-1 px-2 balloon-text-dynamic" label small color="cyan"
                :class="$vuetify.theme.dark ? 'theme--dark darken-3' : 'theme--light lighten-3'" v-bind="attrs"
                v-on="on">
    ...
    <v-card-actions class="justify-center">
      <v-btn :color="feature.active ? 'cyan' : 'secondary'" :disabled="!canFreeze"
        @click="toggleActive"><v-icon>mdi-snowflake</v-icon></v-btn>
    </v-card-actions>
  </v-card>
</template>
    ...
    subfeatures() {
      ...
      [this.$store.state.system.features[this.name].unlock, ...this.$store.state.system.features[this.name].subfeatures].forEach((unlock, index) => {
        if (unlock === null || this.$store.state.unlock[unlock].see) {
          ...
          arr.push({
            subfeature: index,
            isCurrent: this.$store.getters['system/checkExtraCheated']('allSubfeatureCryolab') ? true : this.$store.state.system.features[this.name].currentSubfeature === index,
            level,
            exp,
            expNeeded,
