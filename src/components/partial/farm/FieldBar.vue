  ...
  <div class="d-flex justify-center" style="min-height: 76px;">
    <div>
      <div class="d-flex flex-wrap justify-center my-1" :class="{ 'mx-1': $vuetify.breakpoint.smAndUp }">
        <gb-tooltip :min-width="0">
          <template v-slot:activator="{ on, attrs }">
            <v-btn data-cy="farm-plant-all" class="ma-1" min-width="36" :disabled="!selectedCropName || isFrozen"
              :color="selectedColor ? selectedColor : 'primary'" @click="plantAll" v-bind="attrs"
              v-on="on"><v-icon>mdi-seed</v-icon></v-btn>
          </template>
          <div class="mt-0">{{ $vuetify.lang.t(`$vuetify.farm.button.plantAll`, selectedCropName ?
            $vuetify.lang.t(`$vuetify.farm.crop.${selectedCropName}`) : '') }}</div>
          <alert-text v-if="selectedColor" :type="selectedColor" icon-name="info">{{
            $vuetify.lang.t(`$vuetify.farm.button.colorFilter`) }}</alert-text>
        </gb-tooltip>
        <gb-tooltip :min-width="0">
          <template v-slot:activator="{ on, attrs }">
            <v-btn data-cy="farm-replant-all" class="ma-1" min-width="36" :disabled="isFrozen"
              :color="selectedColor ? selectedColor : 'primary'" @click="replantAll" v-bind="attrs"
        ...
        <gb-tooltip :min-width="0">
          <template v-slot:activator="{ on, attrs }">
            <v-btn data-cy="farm-harvest-all" class="ma-1" min-width="36" :disabled="isFrozen"
              :color="selectedColor ? selectedColor : 'primary'" @click="harvestAll" v-bind="attrs"
              v-on="on"><v-icon>mdi-basket</v-icon></v-btn>
          </template>
        ...
        <gb-tooltip :min-width="0">
          <template v-slot:activator="{ on, attrs }">
            <v-btn data-cy="farm-delete" class="ma-1" min-width="36" :disabled="isFrozen"
              :color="deleting ? 'error' : 'secondary'" @click="deleteMode" v-bind="attrs"
        ...
        <gb-tooltip :min-width="0">
          <template v-slot:activator="{ on, attrs }">
            <v-btn class="ma-1" min-width="36"
              :color="selectedColor ? selectedColor : (selectedColor === 0 ? 'error' : 'secondary')"
              @click="toggleColors" v-bind="attrs" v-on="on"><v-icon>mdi-palette</v-icon></v-btn>
          </template>
        ...
        <gb-tooltip :min-width="0">
          <template v-slot:activator="{ on, attrs }">
            <v-btn class="ma-1" min-width="36"
              :color="showSummary ? (selectedColor ? selectedColor : 'primary') : 'secondary'"
              @click="showSummary = !showSummary" v-bind="attrs"
              v-on="on"><v-icon>mdi-chart-box-outline</v-icon></v-btn>
          </template>
      ...
      <div v-if="showColors" class="d-flex flex-wrap justify-center ma-1">
        <v-btn x-small min-width="24" v-for="color in colors" :key="color" class="mr-1" :color="color"
          @click="selectColor(color)"></v-btn>
        <v-btn x-small min-width="24" class="ml-2 px-0" :color="selectedColor === 0 ? 'error' : 'secondary'"
          @click="selectColor(0)"><v-icon small>mdi-delete</v-icon></v-btn>
      </div>
    ...
    <gb-tooltip v-if="unlock.farmCare.use" :title-text="$vuetify.lang.t('$vuetify.farm.care.wateringCan')">
      <template v-slot:activator="{ on, attrs }">
        <div class="watering-can-container mx-2"
          :class="{ 'watering-can-container-mobile mt-2': $vuetify.breakpoint.xsOnly }" v-bind="attrs" v-on="on">
          <v-icon :size="wateringCanSize" color="secondary" class="watering-can-outline"
            :class="{ 'opacity-half': wateringCanFillLevel <= 0 }">mdi-watering-can</v-icon>
          <div class="watering-can-bg" :style="`top: ${wateringCanShift}px;`">
            <v-icon :size="wateringCanSize" :color="wateringCanColor"
              class="watering-can-bg-inner">mdi-watering-can</v-icon>
          </div>
        </div>
      </template>
      <div>{{ $vuetify.lang.t('$vuetify.farm.care.description1', $formatInt(maxCare)) }}</div>
      <div>{{ $vuetify.lang.t('$vuetify.farm.care.description2', $formatNum(rainwaterGain, true),
        $formatInt(rainwater.cap))
        }}</div>
